...
export function strokeRectangle({ color, virtualArea }: { color: string; virtualArea: LeftTopAlignedArea }): Brush {
    return (dest): void => {
        ...
        setCoordinateSystem(dest.canvasContext, {
            center: dest.options.reversed ? minus(dest.canvasSize, physical.center) : physical.center,
        });

        dest.canvasContext.strokeRect(physical.leftTop.x, physical.leftTop.y, physical.size.x, physical.size.y);
    };
}
